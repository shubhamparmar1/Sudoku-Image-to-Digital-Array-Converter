...
def image_ip(image_path):
    if not os.path.isfile(image_path):
        raise FileNotFoundError(f"The file {image_path} does not exist.")
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError(f"The file {image_path} could not be opened as an image.")
...
sudoku_grid = []
def generate_sudoku_board(cropped):
    sudoku_grid = []
...
for row in sudoku_grid:
>>>>>>> 678d9570265bee010235ef4896edf0281763ab95
    print(row)